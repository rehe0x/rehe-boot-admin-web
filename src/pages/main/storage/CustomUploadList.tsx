...
interface CustomUploadListProps {
  ...
  onStatus: (fileId: string, status: string) => Promise<void>;
}

const CustomUploadList: React.FC<CustomUploadListProps> = ({
  fileList = [],
  onDelete,
  onStatus,
}) => {
          ...
          <div>
            {item.status === "error" ? (
              <a
                style={{
                  color:
                    item.status === "error" ? "var(--ant-color-error)" : "",
                }}
              >
                Error
              </a>
                ...
                {(item.status === "uploading" || item.status === "pause") && (
                  <a
                    onClick={() =>
                      onStatus(
                        item.fileId,
                        item.status === "uploading" ? "pause" : "uploading"
                      )
                    }
                  >
                    {item.status === "uploading" ? "暂停" : "继续"}
                  </a>
                )}

                {item.status === "error" && (
                  <a onClick={() => onStatus(item.fileId, "restart")}>重试</a>
                )}
